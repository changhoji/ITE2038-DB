...
void print_times() {
    ...
    for (int i = 0; i < 25; i++) {
        printf("(%08.05lf%%)%30s:  %lfs  (cnt = %d)\n", (double)times[i]/sum*100, names[i], times[i], counts[i]);
    }
...
H_P * load_header(off_t off) {
    ...
    if (sizeof(H_P) > pread(fd, newhp, sizeof(H_P), 0)) {
        ...
        double tmp = (double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000;
        if (tmp >= 0)
            times[LOAD_HEADER] += tmp;
        return NULL;
    }
    counts[LOAD_HEADER]++;
    clock_gettime(CLOCK_MONOTONIC, &end);
...
page * load_page(off_t off) {
    ...
    if (tmp >= 0)
        times[LOAD_PAGE] += tmp;
    // printf("now: %lf, timesum = %lf\n",(double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000, times[LOAD_PAGE]);
    return load;
}
...
off_t insert_into_parent(off_t old, int64_t key, off_t newp) {
    ...
    if (parent->num_of_keys < INTERNAL_MAX) {
        ...
        if (tmp >= 0)
            times[INSERT_INTO_PARENT] += tmp;
        // printf("end: (%ld, %ld), begin: (%ld, %ld), now: %lf, timesum = %lf\n", end.tv_sec, end.tv_nsec, begin.tv_sec, begin.tv_nsec, (double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000, times[INSERT_INTO_PARENT]);
...
off_t insert_into_new_root(off_t old, int64_t key, off_t newp) {
    ...
    double tmp = (double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000;
    if (tmp >= 0)
        times[INSERT_INTO_NEW_ROOT] += tmp;
    return new_root;
...
off_t insert_into_internal(off_t bumo, int left_index, int64_t key, off_t newp) {
    ...
    double tmp = (double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000;
    if (tmp >= 0)
        times[INSERT_INTO_INTERNAL] += tmp;
    return hp->rpo;
}
...
off_t insert_into_internal_as(off_t bumo, int left_index, int64_t key, off_t newp) {
    struct timespec begin, end;
    clock_gettime(CLOCK_MONOTONIC, &begin);
    counts[INSERT_INTO_INTERNAL_AS]++;
    ...
    double tmp = (double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000;
    if (tmp >= 0)
        times[INSERT_INTO_INTERNAL_AS] += tmp;
...
void delete_entry(int64_t key, off_t deloff) {
    ...
    //printf("%d %d\n",why, max);
    if (why <= max && not_enough->num_of_keys == 0) { //delay
        free(not_enough);
        free(parent);
        free(neighbor);
...
void remove_entry_from_page(int64_t key, off_t deloff) {
    ...
    if (lp->is_leaf) {
        ...
        if (deloff == hp->rpo) {
            ...
            double tmp = (double)((end.tv_sec - begin.tv_sec) + (end.tv_nsec - begin.tv_nsec))/1000000000;
            if (tmp >= 0)
                times[REMOVE_ENTRY_FROM_PAGE] += tmp;
            return;
        }
